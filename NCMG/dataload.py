...
# args={"IRS_elements":9,"IRS_elements_row":3,"BS_row":2,"BS_col":3,"BS_antenna":6,"num_users":3,"P_max":dBm_watt(30),"noise_pow":dBm_watt(-174),
...
X_wireless=torch.cat((X_final1,angle_ur,angle_ub,dist_ur,dist_ub,angle_ubr),1)
...
class CustomDataset(Dataset):
    def __init__(self, X_visual, X_wireless):
        self.X_visual = X_visual
        self.X_wireless = X_wireless

    def __len__(self):
        return self.X_visual.size(0)
...
X_test_visual = X_visual[args.get("train_s") + args.get("validation_s"):args.get("train_s") + args.get("validation_s") + args.get("test_s"), :]
X_test_wireless = X_wireless[args.get("train_s") + args.get("validation_s"):args.get("train_s") + args.get("validation_s") + args.get("test_s"), :]

# Create Dataset instances
train_dataset = CustomDataset(X_train_visual.to(device), X_train_wireless.to(device))
valid_dataset = CustomDataset(X_valid_visual.to(device), X_valid_wireless.to(device))
test_dataset = CustomDataset(X_test_visual.to(device), X_test_wireless.to(device))

# Create DataLoaders
train_loader = DataLoader(train_dataset, batch_size=args["batch"], shuffle=False, drop_last=True)
valid_loader = DataLoader(valid_dataset, batch_size=args["batch"], shuffle=False, drop_last=True)
test_loader = DataLoader(test_dataset, batch_size=args["batch"], shuffle=False, drop_last=True)

# Save DataLoaders to args for later use
